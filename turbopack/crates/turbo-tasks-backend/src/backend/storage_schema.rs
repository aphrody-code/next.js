...
impl TaskStorage {
    ...
    pub fn evictability(&self) -> (KeyEvictability, ValueEvictability) {
        ...
        // Already fully evicted in a prior cycle. Classify *why* the task is
        // still in the shard (vs. having been erased outright) so the metrics
        // tell us whether this is the benign `current_session_clean`-only case
        // or something unexpected we need to fix. NOTE: new tasks have the
        // restored flags set as part of construction so the only way for a
        // task to end up in this situation is through eviction.
