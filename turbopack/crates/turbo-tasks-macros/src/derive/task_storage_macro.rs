...
fn generate_task_storage_impl(_ident: &Ident, grouped_fields: &GroupedFields) -> TokenStream {
    ...
    let eviction_methods = generate_drop_methods(grouped_fields);
...
fn generate_drop_methods(grouped_fields: &GroupedFields) -> TokenStream {
    ...
    // Generate default assignments for meta-category inline fields
    let mut drop_meta_inline = Vec::new();
    for field in grouped_fields.persistent_inline(Category::Meta) {
        let field_name = &field.field_name;
        drop_meta_inline.push(quote! {
            self.#field_name = Default::default();
        });
    }
    ...
    quote! {
        ...
        impl TaskStorage {
            ...
            /// Drop all persistent meta-category fields, resetting them to defaults.
            ///
            /// This is used for meta-only eviction: the meta can be re-loaded from
            /// the backing storage on next access. Data fields and transient fields
            /// are preserved.
            ///
            /// After calling this, `meta_restored` and `prefetched` flags are cleared
            /// so the next access triggers a restore from the backing storage.
            pub fn drop_meta(&mut self) {
                // Reset meta-category inline fields to defaults
                #(#drop_meta_inline)*

                // Remove all persistent meta-category lazy fields
                self.lazy.retain(|f| !(f.is_persistent() && f.is_meta()));
                self.lazy.shrink_to_fit();
            ...
            /// Drop all persistent data and meta fields in a single pass.
            ///
            /// More efficient than calling `drop_data()` + `drop_meta()` separately
            /// because it scans `lazy` only once. Used when both categories are
            /// evictable but transient state must be preserved (task stays in map).
            ///
            /// After calling this, both `data_restored` and `meta_restored` flags
            ...
            pub fn drop_data_and_meta(&mut self) {
                ...
                // Remove all persistent lazy fields in a single pass
                self.lazy.retain(|f| !f.is_persistent());
                self.lazy.shrink_to_fit();
